...
### Fixed
- Fixed dynamic theme persistence during Livewire page navigations by adding a listener on the `livewire:navigated` event in the app layout.
- Fixed text visibility of selected variant options ('Pilihan: ...') inside the cart sidebar drawer in dark mode by extending Tailwind colors configuration with `405` and `550` gray variants.
- Fixed missing background colors for primary admin buttons (such as 'Simpan Campaign') and invisible unselected product size text in storefront dark mode by extending Tailwind configuration with custom intermediate color scales (`-150`, `-250`, `-605`, `-650`, `-750`, `-850` shades).
