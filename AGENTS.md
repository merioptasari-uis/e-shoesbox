...
- **Isolated Feature Branches**: The agent must operate exclusively on the feature branch created by the user (or checked out via `helper.sh issue checkout`). Creating, switching, pushing, pulling, or merging branches manually is forbidden; these tasks must be handled strictly by the user or through the automated `helper.sh issue checkout/merge` subcommands.
...
- **Strict Issue Alignment (Issue-Driven Development)**: Before initializing a sprint task, the agent MUST inspect the `.agents/issues/` directory for any open issues (status `open`). The agent MUST select one open issue, map its title to the `Current Task Target` and checklist in `memory.md`, and set the status flag to `IN_PROGRESS`. The task commit MUST include an auto-closing pattern (e.g. `closes #XX`, `fixes #XX`, or `resolves #XX`) referencing the issue ID to ensure the closed issue file is staged and committed atomically with the source changes.
  * **Branch Isolation & Merge**: Under industrial Git-flow best practices, the agent **MUST NOT** make code modifications directly on the base branch (e.g. `main`). Instead, the agent MUST execute `./.agents/scripts/helper.sh issue checkout <id>` to automatically create and switch to an isolated feature branch named `issue-<id>-<slug>`. Once the task is fully complete and all tests/validation checks pass, the agent MUST run `./.agents/scripts/helper.sh issue merge <id>` to automatically validate, close the issue, commit, merge the branch into the base branch via a clean non-fast-forward merge, and delete the feature branch.
